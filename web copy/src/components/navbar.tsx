...
const Navbar = () => {
  ...
  useEffect(() => {
    const handleClickOutside = (e: MouseEvent) => {
      ...
      // Cart: ignore clicks on Trash button
      if (cartRef.current && !cartRef.current.contains(target)) {
        const trashClicked = (target as HTMLElement).closest('.trash-btn');
        if (!trashClicked) setCartOpen(false);
      }
    };
...
<nav className="fixed top-0 left-0 right-0 z-50 theme-bg shadow-2xl rounded-b-2xl px-4 md:px-6 py-4 flex items-center justify-between backdrop-blur-md border-b border-white/10">
      ...
      {/* Cart Sidebar */}
      <div className={`fixed top-0 right-0 w-full max-w-xs md:max-w-md md:w-72 h-full bg-black bg-opacity-100 shadow-2xl z-50 p-6 transition-transform duration-300 border-l border-white/10
        ${cartOpen ? "translate-x-0" : "translate-x-full"}`}>
        <div className="flex justify-between items-center mb-4">
          <h2 className="font-semibold text-lg theme-text">Your Cart</h2>
          <X className="cursor-pointer theme-text-muted hover:theme-text transition-colors" size={18} onClick={() => setCartOpen(false)} />
        </div>

        {cartItems.length === 0 ? (
          <p className="theme-text-muted text-sm">Your cart is empty</p>
        ) : (
          <>
            <div className="space-y-3 bg-black">
              {cartItems.map((item: CartItem) => (
                <div key={item.id} className="flex gap-3 items-center border-b border-white/10 pb-2">
                  {item.image && <img src={item.image} className="w-12 h-12 object-cover rounded" alt={item.name} />}
                  <div className="flex-1">
                    <p className="text-sm font-medium theme-text">{item.name}</p>
                    <p className="text-xs theme-text-muted">{String(item.price)}</p>
                    <div className="flex items-center gap-2 mt-1">
                      <span className="text-xs theme-text-muted">Qty</span>
                      <input
                        type="number"
                        min={1}
                        value={item.quantity}
                        onChange={e=>updateQuantity(item.id, Math.max(1, Number(e.target.value)))}
                        className="w-16 border border-white/20 rounded px-2 py-0.5 text-sm theme-surface theme-text bg-transparent"
                      />
                    </div>
                  </div>
                  <Trash2
                    className="text-red-400 cursor-pointer trash-btn hover:text-red-300 transition-colors"
                    size={18}
            ...
            <div className="mt-4 flex justify-between text-sm font-semibold theme-text">
              <span>Total</span>
              <span>${total.toFixed(2)}</span>
            </div>

            <button
              className="mt-6 w-full btn-primary py-2 rounded-md text-sm"
              onClick={() => open(cartItems)}
            >
              Checkout
            </button>
          </>
